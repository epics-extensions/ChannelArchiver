...
void Engine::shutdown ()
{
	...
	list<ChannelInfo *>::iterator channel = lockChannels ().begin();
...
{
	list<GroupInfo *>::iterator group = _groups.begin();
...
void Engine::setWritePeriod (double period)
{
	...
	lockChannels ();
	list<ChannelInfo *>::iterator channel_info = channel_info = _channels.begin();
...
#	endif
	...
	try
	{
		list<ChannelInfo *>::iterator channel_info = _channels.begin();
		while (channel_info != _channels.end())
		{
	...
	{
		LOG_MSG (osiTime::getCurrent() << ": " << "Engine::writeArchive caught" << e.what () << "\n");
	}
	unlockChannels ();
...
#	ifdef CA_STATISTICS
	if (missing_CA != ChannelInfo::_missing_CA_values)
		LOG_MSG ("Missing: " << ChannelInfo::_missing_CA_values << " CA monitors\n");
#	endif
}
