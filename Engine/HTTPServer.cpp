// InternetServer.cpp: implementation of the InternetServer class.
//////////////////////////////////////////////////////////////////////
...
#endif
	int val = 1;
	if (setsockopt (s, SOL_SOCKET, SO_REUSEADDR, (const char *)&val, sizeof(val)))
	{
...
HTTPClientConnection::HTTPClientConnection (SOCKET socket) : fdReg (socket, fdrRead)
{
	_socket = socket;
	_dest = _line;
	++_total;
...
bool HTTPClientConnection::analyzeInput ()
{
	...
	{	// Linear search ?!
		...
		{
			if ((h->path_len > 0  &&  strncmp (path.c_str(), h->path, h->path_len) == 0)
				||
				path == h->path)
			{
#				ifdef LOG_HTTP
...
#				endif
	...
	{
		LOG_MSG ("HTTPClientConnection: Error in PathHandler for '" << path << "':\n"
			<< e.what());
	}
