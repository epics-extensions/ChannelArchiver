...
bool BinValueIterator::attach (class BinChannel *channel, const stdString &data_file, FileOffset header_offset, bool last_value)
{
	_channel = channel;
	DataFile *file = DataFile::reference (data_file);
...
bool BinValueIterator::readValue (size_t index)
{
	...
	_value->read (_header.getFd (),	_header.getDataOffset() + index * _value->getRawValueSize());
	_value_index = index;
	return true;
}
...
bool BinValueIterator::readCtrlInfo ()
{
	try
	{
		_ctrl_info.read (_header.getFd (), _header->getConfigOffset ());
