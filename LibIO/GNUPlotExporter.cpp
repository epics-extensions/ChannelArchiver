...
void GNUPlotExporter::prolog (std::ostream &out)
{
...
void GNUPlotExporter::post_scriptum (const stdVector<stdString> &channel_names)
{
    stdString dir;
    stdString data;
    stdString image;
    ...
    std::ofstream script (_script_name.c_str ());
#   ifdef __HP_aCC
    if (script.fail())
#   else
    if (!script.is_open())
