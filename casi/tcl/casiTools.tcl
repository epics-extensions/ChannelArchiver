...
package provide casiTools 1.0

########################################################
#
#//PACKAGE casiTools.tcl
#//
#// <H2>Initialization file for CASI
#//     (Channel Archiver Scripting Interface)</H2>
#//
#//
#// This file has to be configured for your system so that it
#// knows how to find the actual CASI extension.
#// <P>
#// When this is done properly, casiTools.tcl...
#// <UL>
#// <LI>loads the extension
...
if { $tcl_platform(platform) == "windows" } {
	...
	load /home/ade/epics/supTop/extensions/R3.14.1_RH8/src/ChannelArchiver/casi/tcl/O.linux-x86/casi.so
    }
}

# Solaris (LEDA@LANL)
# - nothing to load as long as shared lib. cannot be build
# The current tcl-shell was hopefully build with casi hardlinked in.
...
proc formatValue { value } {
    if { [ $value valid ] } {
	if { [ $value isInfo ] } {
	    return "[ $value time ] [ $value status ]"
	}
	return "[ $value time ] [ $value text ] [ $value status ]"
    }
...
proc showCasiNag {} {
    ...
    toplevel .nag
    label .nag.info0 -text "CASI Version $casi_version"
...
proc createChannelInfo { archiveName channelName } {
	...
	if { ! [ $archive findChannelByName $channelName $channel ] } {
	    ...
	    set first [ $channel getFirstTime ]
	    set last [ $channel getLastTime ]
	}
    }
    rename $channel {}
    rename $archive {}
...
proc openSpreadsheetContext { archiveName startStamp channelNameList contextVar valuesVar statiVar } {
	...
	error "Cannot open archive $archiveName"
    }

    foreach channelName $channelNameList {
	# find channel
	set channelId [ channel ]
	if { ! [ $archiveId findChannelByName $channelName $channelId ] } {
	...
	set value {}
	set status {}
	if { [ $channelId getLastTime ] < $startStamp } {
	    # not really doing cleanup here...
	    return false
	} else {
	    set valueId [ value ]
	    if { [ $channelId getValueBeforeTime $startStamp $valueId ] } {
		if { ! [ $valueId isInfo ] } {
		    set value [ $valueId get ]
		}
		set status [ $valueId status ]
		$valueId next
	    }
	}
...
#####################################################
#//* <B>Syntax:</B> nextSpreadsheetContext contextVar timeStampVar valuesVar statiVar
#//
#// <B>Returns:</B> true if values were found
#//
...
proc nextSpreadsheetContext { contextVar timeStampVar valuesVar statiVar } {
    ...
    if { [ string compare $timeStamp "9999/99/99 99:99:99" ] == 0 } {
	return false
    }
