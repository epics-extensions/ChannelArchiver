...
Now an XML-RPC fault like
"Cannot access data file '20040323' in '/Users/....'"
will be reported.

Note that a misspelled name or the fact that there is
simply no data for the requested time is still not
considered a severe error, since this is quite common,
especially when more than one channel is requested.
Instead of failing the whole request,
only a hint is embedded inside fake type information:
The channel's type information will pretends to be
enumerated with only one state, and that state
would be "<NO DATA or WRONG NAME>".
