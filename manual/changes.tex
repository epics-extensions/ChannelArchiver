...
\item 03/30/2006? --- Version 2.8.0:\\
Like a pendulum swinging back, I've overcome my initial
disappointment with C++ exceptions and added them back
in, because as long as the compiler supports them,
their use eases the error handling:
Detect and describe errors at the low level in the code
where they happen, and pass them up to the high level
code in the form of an exception.

In this conversion process, many unit-tests have been added
and checked under valgrind.

Checks of the nanosecond-portion of time stamps have been added
to avoid the 'assert' aborts found since R3.14.8.

The ExampleSetup is quite different, and described in the manual.
...
``20060110'', the tools will use the data from 
