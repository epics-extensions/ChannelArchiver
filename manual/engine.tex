...
is non-zero, sampling of the whole group will be disabled until the
channel returns to zero.
...
marked as ``disable'' will disable its group whenever it is non-zero,
there is no ``enable'' flag that would enable archiving of a group
whenever the flagged channel is non-zero.
