...
public:
	...
	/**
	*	@param result is the pointer to a memory block to which the address of the AU,
	*	or (if the R tree doesn't store data for the desired interval) -1 is written
	...
	*	@param result is the pointer to a memory block to which the address of the AU is,
	*	or (if the R tree doesn't store data for the desired interval) -1 is written
	*	@return false, if errors occured; true otherwise	
	*/
